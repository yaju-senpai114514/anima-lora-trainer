...
공유 로직은 전부 `src/` 패키지에 있고(dedup `src/dedup.py`, 태깅 `src/tagging.py`,
toml 생성 `src/configgen.py`, 서버/프론트엔드 `src/webui/`), 루트 스크립트는 argparse 만
담당하는 얇은 런처다.
...
바로 태깅/toml 로 넘어간다. 원시(중첩) 데이터셋은 아래 dedup 흐름부터 시작한다.

임베딩/태깅 잡은 한 번에 하나만 돈다(GPU 직렬화). 진행률은 UI 에 표시된다.

#### 원시 dedup 탭 — dataset_raw/ → dataset/ 익스포트

앨범/출처별 폴더가 섞여 있거나 같은 그림의 변형(차분)이 많은 `dataset_raw/<원시>/` 를,
DINOv2-base 의 CLS 임베딩으로 그룹핑/제외/강조한 뒤 `dataset/<name>/repeat_<N>/` 심볼릭 링크
데이터셋까지 **한 번에 익스포트**한다. 같은 그림의 변형을 하나로 묶지 않으면 그 그림만 K배
과대표집되는데, 그걸 이름 규칙이 아니라 이미지 내용(임베딩)으로 잡는다. **원본 파일은 절대
건드리지 않고**(read-only), 현황은 `.dedup/<name>/state.json` 메타데이터로만 관리한다.
...
- **속도**: 임베딩의 실제 병목은 GPU forward(수백 ms)가 아니라 PIL 디코드/리사이즈다 → 로딩을
  ...
  — 수동 삭제 불필요.
- 런치 플래그: `--refresh`(임베딩 캐시 무시), `--batch-size`(기본 16, `--fast` 면 32),
  `--host/--port`, `--open`.
- 익스포트가 끝나면 완료 메시지의 **[데이터셋 탭에서 태깅 →]** 버튼으로 바로 이어진다.
  toml 생성은 repeat_<N> 마다 `num_repeats=N` 서브셋을 찍는다.
...
다운로드) `repeat_<N>` 구조는 자동으로 하위까지 태깅한다.

| 옵션 | 기본 | 설명 |
|------|------|------|
| trigger | `@<이름>` | 캡션 맨 앞 고정 토큰. `keep_tokens=1` 과 맞물려 셔플에서 고정된다. |
| general threshold | 0.35 | general 태그 임계값 |
| character threshold | 0.85 | character 태그 임계값 |
| rating 포함 | off | rating 태그(general/sensitive…)를 trigger 다음에 삽입 (이 경우 toml 의 `keep_tokens=2` 로 — 자동 제안) |
| 덮어쓰기 | off | 기존 `.txt` 도 다시 쓴다 |

- **WD 추론 캐시**: 추론 결과는 `.dedup/wd14_cache.json` 에 (심링크를 푼 실경로의
  mtime+size, 모델, threshold) 키로 캐시된다. dedup 재익스포트가 `repeat_*` 를 갈아엎어
  `.txt` 가 사라져도, 재태깅은 추론 없이 캡션 조립만 다시 하므로 몇 초에 끝난다.
  threshold 를 바꾸면 캐시 미스 → 해당 이미지만 재추론.
- 마지막 태깅 설정(trigger/threshold/rating)은 데이터셋별로 기억돼 다이얼로그에 프리필된다.
...
**캡션 보기** — 썸네일+캡션 그리드. 캡션은 그 자리에서 수정/저장한다(빈 내용으로 저장하면
`.txt` 삭제).

**toml 생성/편집…** — sd-scripts 가 읽는 `dataset/<name>.toml`. **[옵션 → 미리보기]** 가
옵션(해상도 `1024`/`1024,768`, num_repeats(평면용), keep_tokens, shuffle_caption, bucket 3종)
으로 toml 을 생성해 **편집기에 채우고**, 에폭당 학습 이미지 수(반복 반영)·배치별 스텝 수를
보여준다. 편집기에서 직접 수정(멀티해상도 블록 등 수동 커스텀 포함)한 뒤 **[저장]** —
...
배치는 GPU 수에 딸린 실행 시점 설정이지 데이터셋 속성이 아니기 때문이다.
...
> 어긋나면 실행을 거부한다. (구형/수동 toml 에 박혀 있는 경우에만 해당)
...
```
src/                  공유 로직 패키지 — 루트 스크립트는 전부 이걸 감싸는 얇은 런처
  paths.py              경로/확장자 규약 (모든 모듈 공통)
  dedup.py              DINOv2 임베딩 + centroid 그룹핑 상태/익스포트
