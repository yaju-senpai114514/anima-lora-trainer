...
usage() {
  cat <<'EOF'
Usage: ./2_run_training.sh <trigger>
...
# --- 실제 배치를 toml 에서 읽는다 ---
# ★ 배치를 정하는 것은 toml 의 batch_size 다. --train_batch_size 가 아니다.
#   sd-scripts 는 --dataset_config 가 있으면 toml 값을 우선하고
#   (library/config_util.py search_value: [dataset, general, argparse] 순 첫 non-None),
#   --train_batch_size 는 로그/메타데이터에만 쓴다(train_network.py 의 total_batch_size /
#   ss_batch_size_per_device). 둘이 어긋나면 실제와 다른 배치가 로그에 찍히므로 검증한다.
...
if [ -z "${BATCH_SIZE:-}" ]; then
  echo "[error] config.${TRIGGER}.env 에 BATCH_SIZE 가 없습니다." >&2
  exit 1
fi
...
# --- 배치 검증: 1-GPU 이므로 toml 의 batch_size == BATCH_SIZE 여야 한다 ---
if [ "$LOCAL_BATCH" -ne "$BATCH_SIZE" ]; then
  echo "[error] 배치가 맞지 않습니다 (1-GPU)." >&2
  echo "        toml 의 batch_size (= 실제 배치)  : $LOCAL_BATCH   ($(basename "$TOML"))" >&2
  echo "        config.${TRIGGER}.env 의 BATCH_SIZE : $BATCH_SIZE" >&2
  echo "" >&2
  ...
  echo "          ./2_run_training_2gpu.sh (GPU당 $(( BATCH_SIZE / 2 )))  /  ./2_run_training_4gpu.sh (GPU당 $(( BATCH_SIZE / 4 )))" >&2
  exit 1
fi
...
echo "Starting LoRA training for <$TRIGGER>  " \
     "(batch=$BATCH_SIZE epochs=$EPOCHS lr=$LR save_every=$SAVE_EVERY_N_EPOCHS gc=${GRADIENT_CHECKPOINTING:-1}" \
     "seed=${SEED:-random} toml=$(basename "$TOML"))..."

# 출력 디렉토리: config 의 OUT_DIR 로 오버라이드 가능(절대경로 또는 REPO 기준 상대). 미설정 시 기본 규칙(gb=글로벌배치).
if [ -n "${OUT_DIR:-}" ]; then
  case "$OUT_DIR" in
    /*) OUT="$OUT_DIR" ;;
    *)  OUT="$REPO/$OUT_DIR" ;;
